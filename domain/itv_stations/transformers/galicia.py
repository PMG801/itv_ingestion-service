...
class GaliciaTransformer(BaseTransformer):
    ...
    def transform(self, raw_payload: Any) -> list[NormalizedStation]:
        ...
        if isinstance(raw_payload, dict):
            ...
            if "stations" in payload_dict or "estaciones" in payload_dict:
                stations_list_obj: object = payload_dict.get("stations") or payload_dict.get(
                    "estaciones"
                )
    ...
    def _transform_station(self, data: Mapping[str, object]) -> NormalizedStation | None:
        ...
        raw_id = (
            self._as_optional_str(data.get("id") or data.get("codigo") or data.get("station_id"))
            or ""
        )
        ...
        try:
            station = NormalizedStation(
                ...
                name=self._as_optional_str(
                    data.get("nome") or data.get("nombre") or data.get("name")
                )
                or "",
                address=self._as_optional_str(
                    data.get("enderezo") or data.get("direccion") or data.get("address")
                ),
                city=self._as_optional_str(
                    data.get("concello")
                    or data.get("poblacion")
                    or data.get("ciudad")
                    or data.get("city")
                ),
                ...
                postal_code=self._clean_postal_code(
                    self._as_optional_str(
                        data.get("cp") or data.get("codigo_postal") or data.get("postal_code")
                    )
                ),
                latitude=self._parse_float(
                    data.get("lat") or data.get("latitud") or data.get("latitude")
                ),
                longitude=self._parse_float(
                    data.get("lon") or data.get("longitud") or data.get("longitude")
                ),
                phone=self._clean_phone(
                    self._as_optional_str(data.get("telefono") or data.get("phone"))
                ),
