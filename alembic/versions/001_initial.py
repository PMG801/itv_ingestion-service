...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "fuente_origen", sa.String(length=50), nullable=False, comment="Sistema fuente de datos"
        ),
        sa.Column(
            "id_en_fuente",
        ...
        sa.Column(
            "nombre", sa.String(length=255), nullable=False, comment="Nombre de la estación ITV"
        ),
    ...
    op.create_index(
        "idx_estaciones_fuente", "estaciones", ["fuente_origen"], unique=False, schema="itv"
    )
    op.create_index(
        "idx_estaciones_cp", "estaciones", ["codigo_postal"], unique=False, schema="itv"
    )
    ...
    op.create_index(
        "idx_ingestion_log_message_id", "ingestion_log", ["message_id"], unique=False, schema="itv"
    )
    op.create_index(
        "idx_ingestion_log_status", "ingestion_log", ["status"], unique=False, schema="itv"
    )
    op.create_index(
        "idx_ingestion_log_processed_at",
        "ingestion_log",
        ["processed_at"],
        unique=False,
        schema="itv",
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS trigger_update_estaciones_fecha_actualizacion ON itv.estaciones"
    )
